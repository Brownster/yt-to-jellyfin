...
def create_music_album_folder(
    ...
    base_folder = Path(app.config["output_dir"]) / "Music"
    if artist_name:
    ...
    if result.returncode != 0:
        ...
        if job:
            job.update(
                status="failed",
                stage="failed",
                detailed_status=f"ffmpeg failed converting {source.name}",
                message=f"ffmpeg failed converting {source.name}",
            )
        raise RuntimeError(f"ffmpeg failed converting {source}")
