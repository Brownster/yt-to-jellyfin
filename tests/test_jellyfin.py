...
class TestJellyfinIntegration(unittest.TestCase):
    ...
    def test_copy_to_jellyfin_creates_and_copies(self):
        ...
        with patch('os.path.exists', side_effect=exists_side) as mock_exists, \
            ...
            mock_makedirs.assert_has_calls([
                call(dest_show, exist_ok=True),
                call(dest_season, exist_ok=True)
            ])
            expected_calls = [
                call(self.source_folder / 'Test Show S01E01.mp4', dest_season / 'Test Show S01E01.mp4'),
                call(self.source_folder / 'Test Show S01E01.nfo', dest_season / 'Test Show S01E01.nfo'),
                call(self.source_folder / 'Test Show S01E01.jpg', dest_season / 'Test Show S01E01.jpg'),
                call(Path(self.temp_dir) / 'Test Show' / 'tvshow.nfo', dest_show / 'tvshow.nfo'),
                call(Path(self.temp_dir) / 'Test Show' / 'poster.jpg', dest_show / 'poster.jpg'),
            ]
            ...
            self.assertTrue(any('Successfully copied all files' in m['text'] for m in self.job.messages))
            mock_scan.assert_not_called()

    def test_trigger_jellyfin_scan(self):
        self.app.config['jellyfin_api_key'] = 'token'
        url = f"http://localhost:8096/Library/Refresh?api_key=token"
        with patch('requests.post', return_value=MagicMock(status_code=204)) as mock_post:
            ...
            self.assertTrue(any('Successfully triggered Jellyfin library scan' in m['text'] for m in self.job.messages))
